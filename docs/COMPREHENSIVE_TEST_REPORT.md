...
|---------|-----------|-----------|--------|------|
| 前端单元测试 (Vitest) | 14 | 202 | 100% | ✅ 通过 (mock测试) |
| 前端 Lint (ESLint) | - | 0 errors | - | ✅ 通过 |
| 前端构建 (tsc + vite) | - | - | - | ✅ 通过 |
| Rust 静态分析 (clippy) | - | 0 errors | - | ✅ 通过（修复 7 个） |
| Rust 编译检查 (cargo check) | - | - | - | ✅ 通过 |
| Rust 运行时测试 (cargo test) | - | - | - | ⚠️ Windows DLL 问题 |
| API 命令层覆盖率审计 | - | - | 67.3% | ⚠️ 17 个死命令 |
| i18n 翻译完整性审计 | - | - | - | ⚠️ 存在问题 |

**综合评估**: 前端测试体系包含 202 个用例，覆盖主要组件和状态管理。后端代码通过 clippy 静态分析，但运行时测试因 Windows DLL 问题无法执行。存在 API 层断层（17 个死命令）和 i18n 结构性问题需后续处理。

> **重要说明**: 本报告中的测试均为前端单元测试（基于 mock），非集成测试或端到端测试。后端 Rust 测试在当前环境无法运行。
...
| 模块 | 内联测试 | 覆盖状态 |
|------|---------|---------|
| core/ (14 文件) | ✅ 包含 #[cfg(test)] 模块 | 代码存在，但未实际运行验证 |
| commands/ (12 文件) | ✅ 包含 #[cfg(test)] 模块 | 代码存在，但未实际运行验证 |
| models/ (3 文件) | ✅ 包含 #[cfg(test)] 模块 | 代码存在，但未实际运行验证 |
| utils/ (3 文件) | ✅ 包含 #[cfg(test)] 模块 | 代码存在，但未实际运行验证 |
| 集成测试 | ⚠️ 2 文件 20 用例 | 因 Windows DLL 问题无法运行 |

> **注意**: 后端测试模块存在于源码中，但因 Windows 环境 `STATUS_ENTRYPOINT_NOT_FOUND` 错误无法执行。上述"完全覆盖"指测试代码存在，而非实际验证通过。
